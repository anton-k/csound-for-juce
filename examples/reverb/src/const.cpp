...
// size of the reverb, how much frequencies of reflections are not dumped, and
// mix for dry/wet ratio. Single choice parameter controlls reverb type.
ParameterSpec init_parameter_spec() {
  ...
  std::vector<AudioParameterChoiceSpec> audio_choices_spec = {
      AudioParameterChoiceSpec(names::reverb_type, "Reverb type",
                               {"Sean Costello", "Freeverb", "Nverb"}, 1)};
  ...
  return ParameterSpec{.audio_floats = audio_floats_spec,
