...
// Choice parameter definition. Note that parameter index is 1-based. Because 0
// is reserved in the JUCE for "no selection".
void AudioPluginAudioProcessorEditor::setup_reverb_type_choice() {
...
void AudioPluginAudioProcessorEditor::setup_knobs() {
...
/// Setup height and width fir the UI window (those params are persisted in the
/// plugin state)
void AudioPluginAudioProcessorEditor::setup_window() {
  int window_height =
      static_cast<int>(processorRef.get_parameters()
                           .get_ui_parameter(names::window_height)
                           .value_or(250.0));
  int window_width = static_cast<int>(processorRef.get_parameters()
                                          .get_ui_parameter(names::window_width)
                                          .value_or(400.0));
  setSize(window_width, window_height);
  setResizable(true, true);

  /// Setup constrainer to keep fixed ratio of the window width and height
  constrainer->setFixedAspectRatio(static_cast<float>(window_width) /
...
/// Setup parameter attachments. It links UI-control to the update of Csound
/// parameters
void AudioPluginAudioProcessorEditor::setup_parameter_attachments() {
...
void AudioPluginAudioProcessorEditor::setup_logger() {
  ...
  time_t now;
  log_consumer->enable_file_logging(
      log_dir.getChildFile(std::format("csound_log-{}.txt", now)),
      "Plugin Started");
  ...
  log_consumer->set_ui_callback(
      [this](const juce::String &msg, csd_plugin::LogLevel level) {
...
int scale_int(float ratio, int value) { return round(ratio * value); }
...
void AudioPluginAudioProcessorEditor::resized() {
  ...
  processorRef.get_parameters().set_ui_parameter(names::window_width,
                                                 local_width);
  processorRef.get_parameters().set_ui_parameter(names::window_height,
                                                 local_height);

  reverb_type_selector.setBounds(
